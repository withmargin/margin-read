// Margin marketing site — runtime interactivity.
// Bundled by Astro and loaded once at end of body via Layout.astro.
...
const I18N: Record<Lang, Dict> = {
  zh: {
    ...
    "hero.cta2": "上架時通知我",
    "hero.cta2m": "Chrome Web Store · 即將上架",
    "hero.email_ph": "你的 email",
    "hero.cta": "送出",
    "hero.note": "無垃圾信、無第三方追蹤。兩個 CTA 同等重要：今天就試，或等上架。",
    ...
    "priv.p1b": "所有 provider 請求由瀏覽器外掛的 service worker 直接送出。中間沒有我們，也沒有任何代理層。",
    "priv.p2t": "不需登入、不需帳號",
    "priv.p2b": "沒有註冊流程、沒有 OAuth、沒有 cloud sync。瀏覽器設定檔本身就是 Margin 的「帳號」。",
    "priv.p3t": "預設無遙測",
    "priv.p3b": "不送 analytics、不送 error reporting、不送 usage events。除錯資訊只在你打開 popup 的 debug mode 時於本地顯示。",
    "priv.p4t": "只送出選定的文字片段",
    "priv.p4b": "Margin 不會把整頁 HTML 丟給 provider，只送偵測到的可讀文字區塊，並批次送出以降低延遲。",
    ...
    "tech.t2": "都是看得見原料的",
    "tech.t3": "。",
    "tech.lede": "Margin 是 TypeScript + Manifest V3，使用 Rolldown 打包。採 MIT 授權，所有原始碼公開。",
    ...
    "price.n2": "Cached pages 成本為 0。Margin 的 persistent cache 讓你重讀已翻譯頁面不會重複扣款。",
  ...
  en: {
    ...
    "hero.cta2m": "Chrome Web Store · soon",
    "hero.email_ph": "you@example.com",
    "hero.cta": "Submit",
    "hero.note":
    ...
    "flow.margin": "Margin extension",
    "flow.toast_sub": "Detect → queue → batch",
...
function applyLang(lang: Lang): void {
  ...
  document
    .querySelectorAll<HTMLButtonElement>(".lang-switch button")
    .forEach((b) => {
...
function currentLang(): Lang {
  return (document.body.dataset.lang as Lang) || "en";
}
...
const translateBtn =
  document.getElementById("translateBtn") as HTMLButtonElement | null;
...
function resetTranslate(): void {
  ...
  if (translateBtn) {
    const span = translateBtn.querySelector("span");
    if (span) {
      span.textContent = I18N[currentLang()]["demo.translate"]!;
    }
  }
}
...
document.querySelectorAll<HTMLButtonElement>(".lang-switch button").forEach((b) => {
  b.addEventListener("click", () => {
    const lang = b.getAttribute("data-lang-set") as Lang | null;
...
signupForm?.addEventListener("submit", (e) => {
  ...
  button.textContent = lang === "zh" ? "已加入名單 ✓" : "On the list ✓";
  button.disabled = true;
  input.disabled = true;
  input.value = "";
  input.placeholder =
    lang === "zh" ? "上架時通知你" : "Will notify you when ready";
});
...
// =========================================
// Init — restore stored lang preference, default English
// =========================================
const stored = STORE?.getItem(LANG_KEY) as Lang | null;
