...
Margin Read 是一个以隐私为优先的双语网页翻译浏览器扩展。
...
Margin 目前仍是早期 MVP，支持 Chrome 与其他 Chromium 浏览器，并使用 Manifest V3。
...
Margin 不包含 PDF 翻译、EPUB 翻译、字幕翻译、OCR、输入框翻译、云端同步、账号系统、社交功能、默认 telemetry，或官方付费翻译额度系统。

## 从源代码安装

Margin 目前尚未上架浏览器扩展商店。请以 unpacked extension 的方式载入：
...
7. 打开一个网页，并从 Margin popup 点击 Translate this page。

## Provider 设置

Margin 不会内置任何 API key。你需要提供自己的原始 provider API key，且不需要加上 `Bearer` 前缀。
...
取得的模型会出现在模型菜单中。如果当前设置的 provider 默认模型或已保存模型没有出现在 provider 返回的列表中，Margin 会保留它作为可选项。

## 隐私

Margin 只会把选取出的文字片段送到你设置的 provider。默认不会送出完整页面 HTML、不需要登录、不使用云端同步，也不包含默认 telemetry。
...
Margin 内置可选的 X-specific detector，适用于 timeline 卡片与长文页面。启用后，它会优先翻译 tweet article 里的 `tweetText` 内容，以及 X article view 里的可阅读区块，而不是扫描所有可见文字节点。

Quoted posts 默认不会翻译，可在 options 中启用。X 已标示为翻译过的帖文默认会被跳过，以避免重复翻译。

## 本地 LLM

Margin 通过 OpenAI Compatible provider 支持本地 LLM runtime。这个 provider 使用 OpenAI 风格的 `/v1/chat/completions` API，允许 API key 留空，并针对本地推理使用较低的默认翻译 concurrency。
...
```text
apps/extension/src/background/     Service worker、provider request、settings 与 cache flow
...
如果页面看起来已启用，但没有插入任何译文，请先在 Margin options 中启用 Debug mode。Popup 会显示当前页面的检测数量、已排入队列的区块、执行中的 request、pending translations、completed translations、错误数量、最新错误，以及检测到的文字示例。
